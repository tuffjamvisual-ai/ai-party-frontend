  ...
  return (
    <div className="min-h-screen bg-[#002633] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px border border-[#405b6b]">
          {departments.map((dept) => (
            <li key={dept.slug} className="">
              <Link
                href={`/departments/${dept.slug}`}
                className="group block h-full p-5 hover:bg-[#002633] transition-colors border-l-2 border-transparent hover:border-l-[#ffffff]"
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  {dept.ministerPhoto ? (
                    ...
                    <div
                      className="w-6 h-6 rounded-full bg-[#002633] flex items-center justify-center text-[13px] text-white"
                      style={{ border: `1px solid ${ACCENT}55` }}
                    >
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {dept.controlZones.length > 3 && (
                    <span className="text-[13px] px-1.5 py-0.5 uppercase tracking-[0.1em] font-semibold rounded-sm text-white bg-[#002633] border border-[#405b6b]">
                      +{dept.controlZones.length - 3}
                    </span>
                  )}
                </div>
