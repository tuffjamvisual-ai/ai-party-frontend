  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] text-white">
      ...
      <main className="max-w-6xl mx-auto px-6 pb-20">
        <div className="border-b border-[#333] pb-8 mb-10">
          <div className="text-xs uppercase tracking-[0.25em] text-[#9a9a9a] mb-3">Policies</div>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight mb-4">AI-generated policy proposals</h1>
          <p className="text-[#cccccc] max-w-2xl leading-relaxed">
          ...
          {policies.map(p => {
            ...
            return (
              <Link
                key={p.id}
                href={`/policies/${p.id}`}
                className="block bg-[#111] border border-[#2a2a2a] hover:border-[#555] transition-colors p-6"
              >
                ...
                <p className="text-sm text-[#bbbbbb] leading-relaxed mb-4 line-clamp-3">{p.summary}</p>
                {total > 0 ? (
                  <>
                    <div className="h-2 bg-[#222] flex overflow-hidden mb-2">
                      <div className="h-full bg-[#4a8a3a]" style={{ width: `${supportPct}%` }} />
                      <div className="h-full bg-[#8a3a3a]" style={{ width: `${opposePct}%` }} />
                    </div>
                  ...
                  <div className="text-xs text-[#777]">No votes yet — be the first.</div>
                )}
              </Link>
            )
          })}
